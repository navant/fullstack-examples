...
export default function Home() {
  ...
  async function getTasks() {
    try {
      const { data, error } = await supabase.from("todo-example").select();
  ...
  async function testSupabaseConnection() {
    try {
      ...
      if (data) {
        alert('Supabase connection is successful')
      }
    } catch (error) {
  ...
  async function addTask() {
    try {
      const { error } = await supabase
        .from("todo-example")
        .insert([{ Name, Activity, StartDate, EndDate }])
        .single();
  
      if (error) throw error;
  
      // Reset task and refresh tasks
  ...
  async function deleteTask(id) {
    await supabase.from("todo-example").delete().eq("id", id); //the id of row to delete
    getTasks();
  }
